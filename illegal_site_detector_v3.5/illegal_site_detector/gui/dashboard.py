...
class IllegalSiteDetector:
    ...
    def _build_header(self):
        ...
        ctk.CTkLabel(title_box, text="Illegal Site Detector  ·  v2.5",
    ...
    def _build_single_tab(self, parent):
        ...
        ctk.CTkButton(row, text="분석", font=self.f_bold, width=110, height=44,
        ...
        ex.pack(fill="x", padx=18, pady=(6, 14))
    ...
    def _run_single(self):
        ...
        self._set_single_status("🔄 크롤링 중...  robots.txt 확인 → 페이지 수집 → 분석")
    ...
    def _single_worker(self, url):
        try:
            ...
            # 네트워크/접근 실패로 내용을 전혀 못 가져온 경우 → 분석 대신 안내
            if crawl_err and not has_content:
                low = str(crawl_err)
                if ("연결" in low) or ("시간 초과" in low) or ("Connection" in low) \
                        or ("Timeout" in low) or ("Max retries" in low):
                    friendly = "네트워크 연결을 확인해 주세요"
                elif "robots" in low:
                    friendly = "robots.txt 정책으로 크롤링이 허용되지 않은 사이트입니다"
                else:
                    friendly = f"페이지를 불러오지 못했습니다 ({low})"
                self.q.put(("single_fail", (url, friendly)))
                return
    ...
    def _set_verdict_pending(self):
        ...
        self.verdict_main.configure(text="분석 중", text_color=TXT_PRI)
        self.verdict_sub.configure(text="페이지를 수집하고 있습니다")
