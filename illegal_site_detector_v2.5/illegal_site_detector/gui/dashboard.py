...
class IllegalSiteDetector:
    ...
    def _build_kisa_tab(self, parent):
        ...
        _label(info, "🛡️  실시간 위협 인텔리전스 연동 (무료 공개 피드)",
               font=FONT_T, bg=BG_CARD).pack(anchor="w", padx=14, pady=(10, 2))
        _label(info,
               "OpenPhish · Phishing.Database · URLhaus 3개 무료 공개 피드를 통합 수신하여 "
        ...
        _label(guide, "📌 무료 공개 피드 사용 안내", font=FONT_B, bg=BG_CARD
    ...
    def _kisa_refresh_worker(self):
        try:
            ...
            invalidate_index()   # 메모리 인덱스 재구축 유도
